...
int main(){
    ...
    int array[size], negatives, zeros, positives;
    ...
    /*I call the functions to determine the quantity of zeros, negative numbers, and positive numbers in the array*/
    negatives = Quantity_of_Negatives(array, size);
    zeros = Quantity_of_Zero(array, size);
    positives = Quantity_of_Positives(array, size);

    /*I display on the screen the result of the previous functions*/
    printf("The array has: %d negatives, %d zeros, and %d positives.\n", negatives, zeros, positives);
...
void Testing(){
    ...
    /*I declare the variables*/
    int vector_n[] ={-1, 2, 1}, vector_n2[] ={-2, -2, -3}, vector_n3[] ={3, 3, 3};
    int vector_z[] ={-1, 2, 1}, vector_z2[] ={0, 0, 0}, vector_z3[] ={0, 3, 3};
    int vector_p[] ={-1, 2, 1}, vector_p2[] ={0, 0, 0}, vector_p3[] ={0, 3, -3};

    /*I am testing the function*/
    assert(Quantity_of_Negatives(vector_n, 3) == 1);
    assert(Quantity_of_Negatives(vector_n2, 3) == 3);
    assert(Quantity_of_Negatives(vector_n3, 3) == 0);
    assert(Quantity_of_Zero(vector_z, 3) == 0);
    assert(Quantity_of_Zero(vector_z2, 3) == 3);
    assert(Quantity_of_Zero(vector_z3, 3) == 1);
    assert(Quantity_of_Positives(vector_p, 3) == 2);
    assert(Quantity_of_Positives(vector_p2, 3) == 0);
    assert(Quantity_of_Positives(vector_p3, 3) == 1);
